...
**Lead Engineer:** LeukQuant Staging Validation Lead  
**Date:** 2026-08-15  
...
| Service Name | Type | Coolify App ID | Staging Port / Domain | Status |
| :--- | :--- | :--- | :--- | :---: |
| **`leukquant-postgres-staging`** | PostgreSQL 16 DB | `srv-pg-staging` | `5432` (Private Internal Network Only) | `Prepared` |
| **`middle-man-1-staging`** | Spring Boot Ingestion | `srv-mm1-staging` | `8081` (Internal / Agent Ingress) | `Prepared` |
| **`middle-man-3-staging`** | Spring Boot Analytics | `srv-mm3-staging` | `8080` (Internal WebSocket / API) | `Prepared` |
| **`admin-backend-staging`** | Express / Node.js | `srv-adm-be-staging` | `8082` (Internal Admin API) | `Prepared` |
| **`admin-dashboard-staging`** | React SPA / Nginx | `srv-adm-fe-staging` | `admin-staging.leukquant.com` | `Prepared` |
| **`client-dashboard-staging`** | React SPA / Nginx | `srv-cli-fe-staging` | `dashboard-staging.leukquant.com` | `Prepared` |
| **`ghostnet-staging-sensor`** | Python Honeypot Daemon | `vps-sensor-01` | `2222` (SSH Listener) | `Prepared` |
...
flowchart TD
    ...
    S3 --> S4[Step 4: Flyway Schema History & User Data View Verification]
    S4 --> S5[Step 5: middle-man-3 Deployment & WebSocket Handshake]
    S5 --> S6[Step 6: Admin Backend & Admin Dashboard Deployment]
    S6 --> S7[Step 7: Real Agent Provisioning via Admin UI]
    S7 --> S8[Step 8: Ghost-Net Sensor Deployment with Staging Token]
...
- **Verification**: `GET /actuator/health` returns `{"status":"UP"}`. WebSocket endpoint `/api/ws` accepts authenticated connections.
- **Status**: `Awaiting Coolify Staging`
...
- **Verification**: Admin login accepts `ADMIN_KEY`, sets `HttpOnly` session cookie, and returns valid CSRF token.
- **Status**: `Awaiting Coolify Staging`

### Step 7: Real Agent Provisioning via Admin UI
- **Action**: Provision sensor agent for `tenant-alpha-test` on `dep-alpha-staging-01`.
- **Verification**:
  - Raw token starting with `lka_` displayed once in modal.
  - Network tab inspection proves `agent_token_hash` is never transmitted to frontend.
  - Provisioning record inserted into audit log.
- **Status**: `Awaiting Coolify Staging`

### Step 8: Ghost-Net Sensor Deployment with Staging Token
- **Action**: Configure Ghost-Net on staging VPS with `COLLECTOR_URL=https://mm1-staging.leukquant.internal/collect/livedata` and provisioned Bearer token.
- **Status**: `Awaiting Coolify Staging`
...
- **Action**: Provision second agent for `tenant-beta-test`. Send cross-tenant events and verify complete isolation across API queries, WebSockets, and dashboard views.
- **Status**: `Awaiting Coolify Staging`
...
======================================================
  ...
  Local Preflight Tests:      PASSED (299/299)
